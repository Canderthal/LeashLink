...
const SportClasses = ({ classType }) => {
    ...
    useEffect(() => {
        const fetchClasses = async () => {
          try {
            let classRef;
              classRef = collection(db, 'LeashLink', 'Services', 'Classes', classType, selectedLevel);
    ...
    return (
        <Container className='trainingClassesContainer'>
            ...
            <Row className='text-center'>
                <h2>{capitalizeFirstLetter(classType)} Classes</h2>
            </Row>

            <Row className='text-center mt-3'>
                <Col>
                  <Button onClick={() => handleLevelSelect("beginner")} color={selectedLevel === "beginner" ? 'primary' : 'secondary'}>Beginner</Button>
                </Col>
                <Col>
                  <Button onClick={() => handleLevelSelect("intermediate")} color={selectedLevel === "intermediate" ? 'primary' : 'secondary'}>Intermediate</Button>
                </Col>
                <Col>
                  <Button onClick={() => handleLevelSelect("advanced")} color={selectedLevel === "advanced" ? 'primary' : 'secondary'}>Advanced</Button>
                </Col>
                <Col>
                  <Button onClick={() => handleLevelSelect("masters")} color={selectedLevel === "masters" ? 'primary' : 'secondary'}>Masters</Button>
                </Col>
              </Row>
